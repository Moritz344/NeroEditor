...
class MarkdownViewerApp(ctk.CTkFrame):
    def __init__(self, root):
        ...
        bold_btn = ctk.CTkButton(self.topbar,width=10,text="Bold")
        bold_btn.place(x=10,y=10)

        kursiv_btn = ctk.CTkButton(self.topbar,width=10,text="Kursiv")
        kursiv_btn.place(x=60,y=10)

        deleted_btn = ctk.CTkButton(self.topbar,width=10,text="//")
        deleted_btn.place(x=120,y=10)
    ...
    def new_file(self,):
        file_path = filedialog.asksaveasfilename(title="Create File",filetypes=[("Markdown","*.md")])
    ...
    def preview(self,window,event=None):
        ...
        html_1 = markdown.markdown(text_input)
